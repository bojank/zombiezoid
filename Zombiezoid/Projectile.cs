...
namespace Zombiezoid
{
    ...
    class Projectile
    {
        ...
        public Projectile(Game game, Player player ,Vector2 startingPosition, float angle)
        {
            this.player = player;
            this.angle = angle;
            this.game = game;
        ...
        public void Update(GameTime gameTime)
        {
            ...
            for (int i = game.hoard.hoard.Count - 1; i >= 0; i--)
            {
                Enemy enemy = game.hoard.hoard[i];

                Vector2 depth = RectangleExtensions.GetIntersectionDepth(bounds, enemy.BoundingRectangle);
                if (depth != Vector2.Zero)
                {
                    enemy.DoDamage(damage);
                    if (!enemy.isAlive)
                    {
                        game.hoard.hoard.Remove(enemy);
                    }
                    this.isStopped = true;
                }
            }
